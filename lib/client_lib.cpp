...
//IDENTIFY Message Sent During Construction 
    char buffer[BUFSIZ];
    char* IDENTIFY_MSG = "IDENTIFY ophelan1 222\n";
    fputs(IDENTIFY_MSG, server_stream);                 // Send the Identify Msg
...
void Client::retrieve(const char * user_id){
	stringstream ss;
...
int main(int argc, char *argv[]) {
    ...
    // Create Client And do Some Stuff With It 
    Client tmp("host", "port", "cid");
    tmp.subscribe("hot_topic");
    tmp.publish("hot_topic", "hello, world", 12);
    tmp.retrieve("ophelan1");
    tmp.unsubscribe("hot_topic");
    // Run the Client 
    tmp.run();
